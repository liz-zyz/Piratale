...
// ----------------------------------------------------------------------------
// Version
// 1.3.0 2021/08/26 プラグインの機能を一時無効化するスイッチを追加
...
(function() {
    ...
    var param = createPluginParameter('PlayerPointerTurnNoUpDown');
    ...
    Game_Player.prototype.turnToPointer = function() {
        ...
        var tx = TouchInput.x, sx = this.screenX();
        var dir;
        if (isExistPlugin('PD_8DirDash') && this._characterName.indexOf('Q') !== -1) {
            var degree = (Math.atan2(0, -(tx - sx)) / Math.PI + 1) / 2 * 8;
