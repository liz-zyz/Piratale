...
/*:
 * @plugindesc 【ver.3.15】マウスやタッチ操作を変更します。
 * @author 木星ペンギン
 *
 * @help プラグインコマンド:
...
TouchInput._onMouseMove = function(event) {
    if (ConfigManager.getMppParam('mppSelectType') === 0) {
...
Window_Selectable.prototype.setTopRow = function(row) {
    var lastScrollY = this._scrollY;
    Alias.WiSe_setTopRow.apply(this, arguments);
    if (ConfigManager.getMppParam('mppSelectType') === 1 &&
...
Window_Selectable.prototype.processTouch = function() {
    if (this.isOpenAndActive()) {
        ...
        } else if (ConfigManager.getMppParam('mppSelectType') === 1 &&
                TouchInput.isMoved()) {
            this.onTouch(false);
        }
...
Window_Selectable.prototype.onTouch = function(triggered) {
    if (triggered) {
        if (ConfigManager.getMppParam('mppOutsideTap') === 1 &&
                !this._touchInsided && !this.isTouchedInsideFrame()) {
            if (this.isCancelEnabled())     this.processCancel();
        } else if (ConfigManager.getMppParam('mppOkType') < 2 ||
                TouchInput.isMppDoubleTap()) {
            TouchInput.clearMppInterval();
            this._stayCount = 0;
        ...
        if (this.index() !== lastIndex) {
            TouchInput.clearMppInterval();
            if (ConfigManager.getMppParam('mppOkType') === 0) {
                this._selecting = false;
            }
