...
(function() {
    ...
    if (Utils.isDesignMode()) {
        ...
        //=============================================================================
        // NW.js 0.29+ 用キーボード入力パッチ
        // Input.keyMapper が正常に動作しないため、keydown を直接フックする
        //=============================================================================
        (function() {
            const pressedKeys = {};
            ...
            document.addEventListener("keydown", function(e) {
                ...
                if (e.ctrlKey && (e.key === "s" || e.key === "S")) {
                    e.preventDefault();
                    Input._currentState['save'] = true;
                }

                // Ctrl+C → コピー
                if (e.ctrlKey && (e.key === "c" || e.key === "C")) {
                    e.preventDefault();
                    Input._currentState['copy'] = true;
                }

                // Ctrl+Z → Undo
                if (e.ctrlKey && (e.key === "z" || e.key === "Z")) {
                    e.preventDefault();
                    Input._currentState['undo'] = true;
                }

                // 数字キー 1〜9 → プロパティ変更
                if (!e.ctrlKey && /^[1-9]$/.test(e.key)) {
                    Input._currentState['num' + e.key] = true;
                }
            });
            ...
            // Input.isTriggered を拡張して新しいキー名を扱えるようにする
