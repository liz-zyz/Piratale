...
Sprite_Damage.prototype.createDigits = function(baseRow, value) {
    ...
    for (var i = 0; i < string.length; i++) {
        ...
        sprite.x= (i - (string.length - 1) / 2) * (w - spacing);
        sprite.dy = -i;
...
Sprite_Damage.prototype.setup = function(target) {
    ...
    if (result.missed || result.evaded) {
        ...
        if (target === $gamePlayer) {
            this.createDigits(6, result.hpDamage);  // プレイヤーの場合は6番で表示
        } else {
            this.createDigits(0, result.hpDamage);  // プレイヤー以外は0番で表示
        }
    } else if (target.isAlive() && result.mpDamage !== 0) {
        this.createDigits(2, result.mpDamage);
    }
    if (result.critical) {
        this.createDigits(5, result.hpDamage);  // クリティカルの場合は5番で表示
    }
};
...
Sprite_Damage.prototype.updateChild = function(sprite) {
    sprite.ry -= 0.5; // 上方向にダメージ表記が移動
    sprite.y = Math.round(sprite.ry);
    
    // サイズを縮小する処理
    var scaleFactor = Math.max(0.5, 1 - ((180 - this._duration) / 180));
    sprite.scale.x = scaleFactor;
    sprite.scale.y = scaleFactor;

    // 間隔を調整する処理（間隔を縮小）
    var spacing = Math.max(2, 4 - (180 - this._duration) / 45);
    sprite.x = sprite.dy * (- sprite.width - spacing + 12) - 32; //一番右の値がx座標をずらすための値
    var fadeStart = -60;
    if (this._duration < 90 - fadeStart) {
        sprite.opacity -= 3;
    }
